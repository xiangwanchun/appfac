...
let Elitepage = React.createClass({
  getInitialState() {
    return {
       titleNum:'0',
   	   title2Num : '0',
    ...
    componentWillMount:function(){
		...
		var  levedata = this.state.levedata;
			 ...
			 levedata.navigate_id_name = this.props.data.navigate_id_name;
    },
  ...
  inputNum(name,Navname,rule, value, callback) {
	...
	render(){
		var  levedata = this.state.levedata;
		//绑定导航栏
		var navlinkcont = this.props.data.navigate_id_name.map(function(ele,index) {
						    ...
						    <Col span='15'>
								<FormItem
								          ...
								          labelCol={{ span:4}}
		      							  wrapperCol = {{ span:18}}
											>
											<span className='Category wrapperColheight'>{navlinkcont}</span>
								</FormItem>			
							</Col>
						</div>
...
Elitepage = createForm()(Elitepage);
